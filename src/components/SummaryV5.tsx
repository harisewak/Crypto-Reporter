...
interface SummaryV5Props {
  version: string;
  summary: Map<string, AssetSummaryV5[]>;
  dateSortDirection: 'asc' | 'desc';
  toggleDateSort: () => void;
}

export const SummaryV5: React.FC<SummaryV5Props> = ({
  version,
  summary,
  dateSortDirection,
  toggleDateSort,
}) => {
  if (version !== 'v5' || summary.size === 0) {
    return null;
  }
  ...
  return (
    <Box sx={{ mt: 4 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
        ...
        <IconButton
          size="small"
          onClick={() => exportV5SummaryToCSV(version, summary)}
